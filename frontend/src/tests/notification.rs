...
pub fn TestNotifications() -> impl IntoView {
    ...
    view! {
        <div style:padding="32px">
            <Block spacing=48 direction=block::Direction::Column style:align-items="start">
                <Block spacing=16 direction=block::Direction::Column style:align-items="start">
                    <Block spacing=16 style:align-items="center">
                        <TextField
                            attr:value=message_content
                            on:change=handle_change_text
                            attr:data-testid="message-input"
                        />
                        <Slider
                            on:change=handle_toggle_fade
                            tooltip="Fade out"
                            attr:data-testid="message-fade-slider"
                        />
                    </Block>
                    <Button on:click=handle_click attr:data-testid="send-message">
                        Send message
                    </Button>
                </Block>
                <Button on:click=handle_error attr:data-testid="send-error">
                    Send Error
                </Button>
            </Block>
        </div>
